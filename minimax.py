...
def get_next_move(board, player, heuristic_method):
    ...
    #debug_file = open('austin_minimax_#debug.txt', 'w')
    #debug_file.write(datetime.datetime.fromtimestamp(datetime.datetime.now().timestamp()).isoformat())
    #debug_file.write('\n\n')

    result = minimax(board, player, depth, 1, -99, heuristic_method)
...
def minimax(board, player, depth, current_level, ab_best, heuristic_method):
    #debugModifer = ''
                ...
                if board[y][x] == current_player:
                    ...
                    for newY in range(0, 8, 1):
                        ...
                        if func.is_valid_move(board, current_player, currentMove):
                            currentHeuristic = minimax(func.make_move(board, current_player, currentMove), player, depth, current_level + 1, bestHeuristic, heuristic_method)
